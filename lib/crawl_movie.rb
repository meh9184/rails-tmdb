...
def generateCrawlList()
    ...
    # 스크립트 실행시 -top 옵션과 상위 몇위 까지 크롤할지의 크기를 입력하면 평점 순위대로 영화를 수집
    if ARGV[0] == '-top' && ARGV.length == 2

        # TMDB API의 GET movie/top_rated 리소스의 경우 각 페이지별로 20개씩 제공하기 때문에 page와 offset을 적절히 계산해야 함
        ...
        for i in 1..number_of_page
            ...
            # TMDB API GET GET movie/top_rated 리소스 URI를 만들어 20개의 영화 데이터 요청 
            tmdb_api_uri_movie_top = URI("https://api.themoviedb.org/3/movie/top_rated?api_key=#{$tmdb_api_key}&language=en-US&page=#{i}")
            movies_top_rated = getDataFromApi(tmdb_api_uri_movie_top)

            # 각 페이지의 모든 영화 ID 순서대로 crawl_list에 저장
            for j in 0..19
                crawl_list.push(movies_top_rated['results'][j]['id'])
            end
        end

        # 미리 계산해놓은 page 만큼 수집을 완료하면 마지막 페이지에서는 미리 계산한 offset 만큼 영화 ID를 수집
        # TMDB API GET GET movie/top_rated 리소스 URI를 만들어 영화 데이터 요청
        tmdb_api_uri_movie_top = URI("https://api.themoviedb.org/3/movie/top_rated?api_key=#{$tmdb_api_key}&language=en-US&page=#{number_of_page+1}")
        movies_top_rated = getDataFromApi(tmdb_api_uri_movie_top)

        # 20개의 데이터 중 상위 offset 개의 영화 ID를 crawl_list에 저장
        for i in 0..(number_of_offset-1)
            crawl_list.push(movies_top_rated['results'][i]['id'])
        end
